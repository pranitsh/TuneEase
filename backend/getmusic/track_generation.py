...
def get_args(input_sequence = None):
    ...
    parser.add_argument('--conditional_tracks', type=str, default = "False, False, False, False, False, False, True")
    parser.add_argument('--content_tracks', type=str, default = "False, False, False, False, False, False, False")
...
def encoding_to_MIDI(encoding, tpc, decode_chord):
    ...
    # TODO: max(set(i), key=i.count) will always result in 1
...
def F(file_name, conditional_tracks, content_tracks, condition_inst, chord_from_single):
    ...
    if conditional_tracks[-1]:
        ...
        with_chord = False
    print("with_chord ", with_chord, sep="")
    ...
    print("Conditional Tracks ", conditional_tracks, sep="")
    
    # for i in range(14):
    #     if with_chord  and ((datum[i] == pad_index).sum() + (datum[i] == empty_index).sum()) == min(512,figure_size):
    #         # Not necessary: and conditional_tracks[i][0] == 1
    #         have_cond = False
    #         break
...
def main(args = None):
    ...
    conditional_track = np.array([item.strip() == 'True' for item in args.conditional_tracks.split(',')])
    condition_inst = []
    if conditional_track[0] == True:
        condition_inst.append('80')
    if conditional_track[1] == True:
        condition_inst.append('32')
    if conditional_track[2] == True:
        pass
    if conditional_track[3] == True:
        condition_inst.append('25')
    if conditional_track[4] == True:
        condition_inst.append('0')
    if conditional_track[5] == True:
        condition_inst.append('48')


    content_track = np.array([item.strip() == 'True' for item in args.content_tracks.split(',')])
