...
class ClipGradNorm(object):
    ...
    def __call__(self, parameters):
        ...
        if self.last_epoch >= self.start_iteration:
            clip = True
        if self.end_iteration > 0 and self.last_epoch < self.end_iteration: # PS. Although the comment above indicates -1 means always clipping, this statement is contradicitonary
            clip = True 
        if clip:
