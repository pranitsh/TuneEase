...
class KeyChordDetails:
    ...
    def __init__(self) -> None:
        if not bool(self.pos_in_bar):
            magenta = Magenta()
            self.pos_in_bar = (
                self.beat_note_factor * self.max_notes_per_bar * self.pos_resolution
            )
            self.figure_size = self.bar_max * self.pos_in_bar
            self.key_profile_path = os.path.join(
            ...
            self.key_chord_transition_distribution = (
                magenta._key_chord_transition_distribution(
                    self.key_chord_distribution,
                    key_change_prob=self.key_change_prob,
                    chord_change_prob=self.chord_change_prob,
                )
            )
            self.key_chord_transition_loglik = np.log(
                self.key_chord_transition_distribution
            )
