...
def F(
    filename,
    file_name,
    conditional_tracks=None,
    content_tracks=None,
    condition_inst=None,
    chord_from_single=None,
):
    t_h = TokenHelper()
    ...
    if filename == "track_generation.py":
        ...
        datum = torch.where(
            ((datum != t_h.empty_index).float() * (1 - conditional_bool)).type(
                torch.bool
            ),
            t_h.empty_index + 1,
            datum,
        ...
        for i in range(14):
            if (
                ...
                and (
                    (datum[i] == t_h.pad_index).sum()
                    + (datum[i] == t_h.empty_index).sum()
                )
                == min(512, figure_size)
            ):
                have_cond = False
        ...
        midi_obj = miditoolkit.midi.parser.MidiFile(file_name)
        encoding, pitch_shift, tpc = MIDI_to_encoding(
            "position_generation.py", midi_obj
        )
        if len(encoding) == 0:
            print("ERROR(BLANK): " + file_name + "\n", end="")
            return None, 0
        ...
        print(
            "The music has {} tracks, with {} positions".format(
                track_set, datum.size()[1]
            )
        )
        ...
        datum = torch.where(
            ((datum != t_h.empty_index).float() * (1 - condition_pos)).type(torch.bool),
            t_h.empty_index + 1,
            datum,
        )
        not_empty_pos = (torch.tensor(np.array(datum)) != t_h.empty_index).float()
        ...
        try:
            ...
            # no empty
            if not all(
                len(i.split()) > tokens_per_note * 2 - 1 for i in output_str_list
            ):
