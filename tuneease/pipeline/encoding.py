...
def MIDI_to_encoding(
    filename, midi_obj, with_chord=None, condition_inst=None, chord_from_single=None
):
    magenta = Magenta()
    ...
    if filename == "track_generation.py":
        ...
        if with_chord:
            ...
            if max_chords > 0:
                chords = magenta.infer_chords_for_sequence(
                    note_items,
        ...
        for j in range(len(pos_to_info)):  # 它这里是不管这个位置有没有音符，都占个位
            ...
            if cnt == 0:
                measure_length = (
                    ...
                    * POS_RESOLUTION
                    // timesignature[1]
                )  # 比如一个3/4的ts，一个4/4的小节有16pos，所以3/4一小节就有12
            pos_to_info[j][0] = bar
            pos_to_info[j][2] = cnt
            cnt += 1
...
def encoding_to_MIDI(filename, encoding, tpc=None, decode_chord=None):
    ...
    if filename == "track_generation.py":
        ...
        pos_to_tempo = [
            list() for _ in range(cur_pos + max(map(lambda x: x[1], encoding)))
        ]
        for i in encoding:
