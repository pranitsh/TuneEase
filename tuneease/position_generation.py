...
def main_worker(local_rank, args):
    ...
    for file_name in file_list:
        ...
        x, tempo, not_empty_pos, condition_pos, pitch_shift, tpc = F(
            "position_generation.py", file_name
        )
        ...
        midi_obj = encoding_to_MIDI(
            "position_generation.py", oct_final, tpc, args.decode_chord
        )

        save_path = os.path.join(
            args.file_path, "position-{}".format(file_name.split("/")[-1])
        )
