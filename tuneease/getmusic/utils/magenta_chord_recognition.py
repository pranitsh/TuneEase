...
class Magenta:
    _PITCH_CLASS_NAMES = None
    _KEY_PITCHES = None
    _CHORD_KIND_PITCHES = None
    _CHORD_KINDS = None
    NO_CHORD = None
    _CHORDS = None
    _KEY_CHORDS = None
    ...
    def _key_chord_distribution(self, chord_pitch_out_of_key_prob):
        ...
        num_pitches_in_key = np.zeros([12, len(self._CHORDS)], dtype=np.int32)
        num_pitches_out_of_key = np.zeros([12, len(self._CHORDS)], dtype=np.int32)
    ...
    def _key_chord_transition_distribution(self, key_chord_distribution, key_change_prob, chord_change_prob):
        ...
        for i, key_chord_1 in enumerate(self._KEY_CHORDS):
            key_1, chord_1 = key_chord_1
            chord_index_1 = i % len(self._CHORDS)

            for j, key_chord_2 in enumerate(self._KEY_CHORDS):
                key_2, chord_2 = key_chord_2
                chord_index_2 = j % len(self._CHORDS)
    ...
    def _chord_pitch_vectors(self):
        """Unit vectors over pitch classes for all chords."""
        x = np.zeros([len(self._CHORDS), 12])
        for i, chord in enumerate(self._CHORDS[1:]):
            root, kind = chord
            for offset in self._CHORD_KIND_PITCHES[kind]:
    ...
    def sequence_note_pitch_vectors(self, sequence, seconds_per_frame):
    ...
    def _key_chord_viterbi(self, chord_frame_loglik,
        ...
        for i, key_chord in enumerate(self._KEY_CHORDS):
