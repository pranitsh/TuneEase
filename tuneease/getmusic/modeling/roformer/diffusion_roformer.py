...
class DiffusionRFM(nn.Module):
    def __init__(
        ...
        *,
        ...
        self.amp = False
        self.num_classes = (
            self.roformer.vocab_size + 1
        )  # defined in vocabulary, add an additional mask
        self.cond_weight = self.roformer.cond_weight
        self.tracks = 14
        self.pad_index = mc.DURATION_MAX * mc.POS_RESOLUTION - 1
        self.figure_size = (
            mc.BAR_MAX * mc.BEAT_NOTE_FACTOR * mc.MAX_NOTES_PER_BAR * mc.POS_RESOLUTION
        )
        ...
        if alpha_init_type == "alpha1":
            at, bt, ct, att, btt, ctt = alpha_schedule(
                self.num_timesteps, N=self.num_classes - 1
            )
        else:
    ...
    def log_sample_categorical_infer(
        self, logits, figure_size
    ):  # use gumbel to sample onehot vector from log probability
        uniform = torch.rand_like(logits)
        gumbel_noise = -torch.log(-torch.log(uniform + 1e-30) + 1e-30)
        ...
        for i in range(self.tracks - 2):  # do not decode chord
            ...
            if i % 2 == 1:  # duration
                track[
                    :, self.pad_index + 1 : -1, :
                ] = -70  # only decode duration tokens
    ...
    def p_sample(
        ...
        if t[0] > 0 and to_sample is not None:
            ...
            _score = torch.where(
                ((1 - condition_pos) * not_empty_pos).type(torch.bool), _score, 0
            )
            # only mask has score
            ...
            for j in range(6):  # do not decode chord
                ...
                out2_idx[0][2 * j * figure_size : (2 * j + 2) * figure_size][
                    sel
                ] = out_idx[0][2 * j * figure_size : (2 * j + 2) * figure_size][sel]
                ...
                sampled[j] += (
                    ...
                    - (
                        log_x_idx[0][2 * j * figure_size : (2 * j + 2) * figure_size]
                        != self.num_classes - 1
                    ).sum()
                ).item()
    ...
    def _train_loss(self, x, condition_pos, not_empty_pos, is_train=True):
        ...
        log_xt = torch.where(
            condition_pos.unsqueeze(1).type(torch.bool), log_x_start, log_xt
        )
        # empty
        log_xt = torch.where(
            (1 - not_empty_pos).unsqueeze(1).type(torch.bool), log_empty, log_xt
        )
        ...
        position_weight = not_empty_pos * (
            (1 - condition_pos) + self.cond_weight * condition_pos
        )  # loss weight: condition/content = cond_weight / 1
        ...
        kl_loss = mask * L_0 + (1.0 - mask) * L_t_1
        ...
        if self.auxiliary_loss_weight != 0 and is_train == True:
            kl_aux = self.multinomial_kl(
                log_x_start[:, :-2, :], log_x0_recon[:, :-2, :]
            )
            kl_aux = kl_aux * position_weight
            kl_aux = sum_except_batch(kl_aux)
            kl_aux_loss = mask * L_0 + (1.0 - mask) * kl_aux
    ...
    def parameters(self, recurse=True, name=None):
        ...
        if name is None or name == "none":
            ...
            param_dict = {
                pn: p for pn, p in self.roformer.named_parameters()
            }  # if p.requires_grad}
    ...
    def forward(
        ...
        **kwargs,
        ...
        if is_train == True:
            log_model_prob, loss, decoder_nll = self._train_loss(
                x, condition_pos, not_empty_pos
            )
            loss = loss.sum()
    ...
    def sample(self, x, tempo, not_empty_pos, condition_pos, skip_step=0, **kwargs):
        batch_size = x.size()[0]
        assert batch_size == 1

        num_to_be_generated_per_track = (x == self.num_classes - 1).sum(-1)[0]
        num_to_be_generated_per_track = (
            num_to_be_generated_per_track[0::2] + num_to_be_generated_per_track[1::2]
        )
        ...
        log_p_x_on_y = -70 * torch.ones(
            (batch_size, self.num_classes, sample_len), device=device
        )
        log_p_x_on_y[:, -1, :] = 0

        start_step = self.num_timesteps

        to_be_sampled_per_step = torch.floor(
            num_to_be_generated_per_track / (self.num_timesteps - 1)
        )

        self.n_sample = to_be_sampled_per_step.unsqueeze(0).repeat(
        ...
        for i in range(6):
            while compensate[i] > 0:
                compensate_freq = (
                    torch.ceil(self.num_timesteps / compensate[i]).long().item()
                )
                if compensate_freq > 1:
                    self.n_sample[0::compensate_freq, i] += 1
                compensate[i] = (
                    num_to_be_generated_per_track[i] - self.n_sample.sum(0)[i]
                )

        self.n_sample = torch.cat(
            [torch.ones(1, 7, device=self.n_sample.device), self.n_sample], dim=0
        )
        ...
        with torch.no_grad():
            for diffusion_index in tqdm(range(start_step - 1, -1, -1)):
                ...
                while (sampled < self.n_sample[diffusion_index]).any():
                    log_p_x_on_y = torch.where(
                        condition_pos.unsqueeze(1).type(torch.bool),
                        log_x_start,
                        log_p_x_on_y,
                    )
                    log_p_x_on_y = torch.where(
                        (1 - not_empty_pos).unsqueeze(1).type(torch.bool),
                        log_empty,
                        log_p_x_on_y,
                    )
                    log_p_x_on_y, sampled = self.p_sample(
                        log_p_x_on_y,
                        t,
                        figure_size,
                        condition_pos,
            ...
            log_p_x_on_y = torch.where(
                condition_pos.unsqueeze(1).type(torch.bool), log_x_start, log_p_x_on_y
            )
            log_p_x_on_y = torch.where(
                (1 - not_empty_pos).unsqueeze(1).type(torch.bool), log_pad, log_p_x_on_y
            )
