...
class Solver(object):
    def __init__(self, gpu, config, args, model, dataloader, logger, is_sample=False):
        ...
        self.logger.debug(
            "Load dictionary: {} tokens.".format(len(self.t_h.ids_to_tokens))
        )
        ...
        if adjust_lr == "none":
            ...
            self.lr = base_lr * math.sqrt(
                args.world_size * config["dataloader"]["batch_size"]
            )
        ...
        self.logger.debug(
            "Get lr {} from base lr {} with {}".format(self.lr, base_lr, adjust_lr)
        )
    ...
    def _get_optimizer_and_scheduler(self, op_sc_list):
        ...
        for op_sc_cfg in op_sc_list:
            ...
            op_cfg = op_sc_cfg.get(
                "optimizer", {"target": "torch.optim.SGD", "params": {}}
            )
            ...
            assert isinstance(
                op_sc["optimizer"]["step_iteration"], int
        ...
        for op_sc_n, op_sc in self.optimizer_and_scheduler.items():
            if phase == "train":
                if op_sc["start_iteration"] > self.last_iter:
                    continue
                if (
                    op_sc["end_iteration"] > 0
                    and op_sc["end_iteration"] <= self.last_iter
                ):
                    continue
            ...
            if phase == "train":
                ...
                if "scheduler" in op_sc:
                    if (
                        ...
                        and (self.last_iter + 1) % op_sc["scheduler"]["step_iteration"]
                        == 0
                    ):
            ...
            loss[op_sc_n] = {
                k: v for k, v in output.items() if ("loss" in k or "acc" in k)
            }
        return loss
                ...
                for op_sc_n, op_sc in self.optimizer_and_scheduler.items():
                    ...
                    for k in op_sc:
                        if k in ["optimizer", "scheduler"]:
                            op_or_sc = {
                                kk: vv for kk, vv in op_sc[k].items() if kk != "module"
                            }
    ...
    def resume(self, path=None, load_optimizer_and_scheduler=True, load_others=True):
        if path is None:
                        ...
                        for kk in op_sc[k]:
                            if kk == "module" and load_optimizer_and_scheduler:
                                self.optimizer_and_scheduler[op_sc_n][k][
                                    kk
                                ].load_state_dict(op_sc[k][kk])
                            elif load_others:  # such as step_iteration, ...
                                self.optimizer_and_scheduler[op_sc_n][k][kk] = op_sc[k][
                                    kk
                                ]
    ...
    def train_epoch(self):
        ...
        for itr, batch in enumerate(self.dataloader["train_loader"]):
            ...
            if (
                ...
                and self.last_iter % self.args.log_frequency == 0
            ):
            ...
            if (
                self.validate_iterations > 0
                and (self.last_iter + 1) % self.validate_iterations == 0
            ):
                if self.logger is not None:
                    self.logger.debug(
                        str("save model for iteration {}".format(self.last_iter))
                    )
    ...
    def infer_sample(
        self, x, tempo, not_empty_pos, condition_pos, use_ema=True, skip_step=0
    ):
        self.model.eval()
        tic = time.time()
        ...
        with torch.no_grad():
            ...
            samples = model.infer_sample(
                x, tempo, not_empty_pos, condition_pos, skip_step=skip_step
            )
            ...
            for t in range(samples.size()[-1]):
                ...
                assert (main_dur <= self.pad_index) and (
                    main_pitch >= self.pad_index
                ), "pitch index is {} and dur index is {}".format(main_pitch, main_dur)
                    ...
                    if p[0] == "M":
                        ...
                        encoding.append(
                            (
                                ...
                                80,
                                p,
                                self.t_h.ids_to_tokens[main_dur][1:],
                                28,
                                ts,
                                tempo,
                            )
                ...
                assert (
                    bass_dur <= self.pad_index and bass_pitch >= self.pad_index
                ), "{}, {}".format(bass_dur, bass_pitch)
                ...
                assert (
                    drums_dur <= self.pad_index and drums_pitch >= self.pad_index
                ), "{}, {}".format(drums_dur, drums_pitch)
                ...
                assert (
                    guitar_dur <= self.pad_index and guitar_pitch >= self.pad_index
                ), "{}, {}".format(guitar_dur, guitar_pitch)
                ...
                assert (
                    piano_dur <= self.pad_index and piano_pitch >= self.pad_index
                ), "{}, {}".format(piano_dur, piano_pitch)
                ...
                assert (
                    string_dur <= self.pad_index and string_pitch >= self.pad_index
                ), "p:{},d:{}".format(string_pitch, string_dur)
                ...
                if (
                    self.t_h.ids_to_tokens[root_id] in self.r_h.root_dict
                    and self.t_h.ids_to_tokens[kind_id] in self.r_h.kind_dict
                ):
