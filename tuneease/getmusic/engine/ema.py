...
class EMA(object):
    def __init__(
        self, model, decay=0.99, update_interval=1, device=torch.device("cpu")
    ):
        self.decay = decay
        ...
        self.cur_state_dict = {
            k: v.clone().to(self.device) for k, v in self.cur_state_dict.items()
        }

    def update(self, iteration):
        if (iteration + 1) % self.update_iterval == 0:
            # print('{} Update ema'.format(iteration))
            if hasattr(self.model, "get_ema_model") and callable(
                self.model.get_ema_model
            ):
            ...
            for k in ema_state_dict.keys():
                ema_state_dict[k] = ema_state_dict[k] * self.decay + cur_state_dict[
                    k
                ].clone().to(self.device) * (1 - self.decay)
    ...
    def modify_to_inference(self):
        ...
        ema_state_dict = {
            k: v.to(self.model.device()) for k, v in ema_state_dict.items()
        }
        if hasattr(self.model, "get_ema_model") and callable(self.model.get_ema_model):
    ...
    def modify_to_train(self):
        self.cur_state_dict = {
            k: v.clone().to(self.model.device()) for k, v in self.cur_state_dict.items()
        }
        if hasattr(self.model, "get_ema_model") and callable(self.model.get_ema_model):
