...
def convert():
    ...
    if app.config["museScore_path"]:
        ...
        try:
            ...
            return send_file(
                output_filepath, as_attachment=True, download_name=output_filepath
            )
        ...
        try:
            ...
            logger.error(
                "Had an error converting the generated content to xml (likely some error with the input file)"
                + str(E)
            )
            return 500
        return send_file(
            fileext + ".xml", as_attachment=True, download_name=fileext + ".xml"
        )
...
def split():
    logger = ServerLogger("tuneease.log").get()
    logger.info(
        "Received a /split request " + str(request.files) + " with " + str(request.form)
    )
    try:
        file = request.files["file"]
        assert file.filename != "", "File is empty"
    ...
    try:
        ...
        try:
            if end_number == "":
                stream = measures[start_number].write(
                    "musicxml", measure_filepath + ".xml"
                )
            else:
            ...
            logger.error(
                f"Your input numbers were outside of the measures of the part (probably): {str(e)}"
            )
            return (
                f"Your input numbers were outside of the measures of the part (probably)",
                500,
            )
        stream.write("musicxml", measure_filepath + ".xml")
        return send_file(
            measure_filepath + ".xml",
            as_attachment=True,
            download_name=measure_filepath + ".xml",
        )
    except Exception as e:
...
def number():
    ...
    output_filepath = os.path.join(
        util.project_directory(),
        "temp",
        os.path.splitext(file.filename)[0] + "-number.xml",
    )
    os.makedirs(os.path.dirname(filepath), exist_ok=True)
    file.save(filepath)
...
def random_score():
    ...
    try:
        ...
        number_measures = int(request.form.get("number_measures", "8")) * int(
...
def generate():
    ...
    if "file" in request.files:
        ...
        assert file.filename != "", "File is empty"
        assert (
            os.path.splitext(file.filename)[1] == ".xml"
            or os.path.splitext(file.filename)[1] == ".mid"
        ), "File is not .xml or .mid"
        util = PathUtility()
        filepath = os.path.join(util.project_directory(), "temp", file.filename)
        os.makedirs(os.path.dirname(filepath), exist_ok=True)
        file.save(filepath)
        ...
        time_signature = request.form.get("time_signature", "4/4")
        quarter_length = 1.0 / (int(time_signature.split("/")[1]) / 4)
        number_measures = int(request.form.get("number_measures", "10")) * int(
            time_signature.split("/")[0]
        )
    ...
    try:
        ...
        return 500
    return send_file(
        save_path + ".xml", as_attachment=True, download_name=save_path + ".xml"
    )
